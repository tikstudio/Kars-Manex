...
import { getNewProductRequest } from "../store/actions/products";
...
class Announcements extends Component {
  ...
  async componentDidMount() {
    ...
    await this.props.getNewProductRequest();
  }
  ...
  handleSelectFilter = async (path, event) => {
    ...
    await this.props.getNewProductRequest(search, formData.sortKey, formData.sortValue,
  ...
  handleSorting = (ev) => {
    ...
    this.setState({formData});
    this.props.getNewProductRequest(search, formData.sortKey, formData.sortValue,
      formData.category_id, formData.category_departmentId, formData.c_section,
    )
  }
  handleSearch = (event) => {
  ...
  search = async (event) => {
    ...
    if (event.code === 'Enter'){
      await this.props.getNewProductRequest(search);
    }
  }

  render() {
    const {category, newProducts, newProductsReqStatus} = this.props;
    const {sortData, formData, search, sorts} = this.state;
    return (
      <AdminWrapper showFooter={ false }>
              ...
              <div className='user__content_row'>
                ...
                <div className='user__product_area'>
                  <div className='userProduct__sort_area'>
                    <p>Ընդհանուր ({ newProducts?.products?.length })</p>
                    <div className='user__search_area'>
                      ...
                      <button className='user__search_button' type='button'
                              onClick={ () => this.props.getNewProductRequest(search) }
                      >Փնտրել
                      </button>
                    </div>
                  ...
                  <div className='user__product_list2'>
                    { newProductsReqStatus === "request" ? <div className="centering"><CircleLoad/></div> :
                      newProductsReqStatus === "success" ?
                        <EditProduct style={ {width: '31%'} } product={ newProducts.products }
                                     thenFunction={ this.props.getNewProductRequest }/> :
                        _.isEmpty(newProducts.products) ? <p className="centering">Հայտարարություն չկա</p> :
...
const mapStateToProps = (state) => ({
  ...
  newProductsReqStatus: state.products.newProductsReqStatus,
